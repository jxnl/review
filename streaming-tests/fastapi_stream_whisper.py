    ...
    **decode_options,
    ...
    async def add_segment(
        *, start: float, end: float, text_tokens: torch.Tensor, result: DecodingResult
        ...
        logger.info(f"adding segment {text}")
    ...
    with tqdm.tqdm(
        ...
        while seek < num_frames:
            ...
            result: DecodingResult = await decode_with_fallback(segment)
...
async def read_from_queue(queue):
    while True:
        logger.info("in the loop waiting for text")
        try:
...
async def stream():
    ...
    audio = whisper.load_audio("../bjj_gpt/data/HalfGuardPassingbyGordonRyan1.mp4.mp3")
    logger.info("Transcribing")
    asyncio.ensure_future(transcribe(model, audio, queue))
    logger.info("Connecting Stream")
    return StreamingResponse(stream_queue(queue), media_type="text/plain")
